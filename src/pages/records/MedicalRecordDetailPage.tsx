...
export default function MedicalRecordDetailPage() {
    ...
    const createAddendumMutation = useMutation({
        mutationFn: (data: { reason: string; content: string }) => {
            return medicalService.createAddendum(id!, data);
        },
        onSuccess: () => {
            toast.success('Thêm bản đính chính thành công');
            setAddendumReason('');
            setAddendumContent('');
            queryClient.invalidateQueries({ queryKey: ['medical-record', id] });
        },
        onError: (err) => {
            console.error(err);
            toast.error(extractErrorMessage(err, 'Lỗi khi tạo bản đính chính'));
        },
    });
    ...
    const tabs = [
        ...
        { id: 'addendum', label: 'Bản đính chính' }
    ];
                                ...
                                <div className="space-y-6">
                                    {record.signedStatus !== 'SIGNED' ? (
                                        <div className="flex flex-col items-center justify-center p-8 text-center bg-gray-50 rounded-lg border border-gray-200">
                                            ...
                                            <p className="text-gray-600 font-medium text-lg">Chưa thể tạo bản đính chính</p>
                                            <p className="text-gray-500 mt-2 max-w-sm">
                                                Bệnh án này chưa được ký số. Việc thêm bản đính chính chỉ áp dụng cho hồ sơ bệnh án đã kết thúc và được bác sĩ ký số điện tử.
                                            </p>
                                        </div>
                                    ) : (
                                        <>
                                            <section>
                                                <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200">
                                                    <h3 className="text-base font-semibold text-gray-900">
                                                        Danh sách các bản đính chính
                                                    </h3>
                                                </div>
                                                {record.addenda && record.addenda.length > 0 ? (
                                                    <div className="space-y-4">
                                                        {record.addenda.map((ad, idx) => (
                                                            <div key={ad.id || idx} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                                                <div className="flex justify-between items-start mb-2">
                                                                    <div className="font-semibold text-gray-900">Lý do: {ad.reason}</div>
                                                                    <div className="text-xs text-gray-500">{new Date(ad.createdAt).toLocaleString('vi-VN')}</div>
                                                                </div>
                                                                <div className="text-sm text-gray-700 bg-white p-3 rounded border border-gray-100">
                                                                    <SafeRichText html={ad.content} />
                                                                </div>
                                                                <div className="mt-2 text-xs text-gray-500">Người đính chính: {ad.doctorName || 'N/A'}</div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    <div className="p-4 text-center text-gray-500 bg-gray-50 rounded border border-gray-200">
                                                        Chưa có bản đính chính nào cho bệnh án này.
                                                    </div>
                                                )}
                                            </section>

                                            <section className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mt-6">
                                                ...
                                                <div className="space-y-4">
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            placeholder="Mô tả tóm tắt lý do..."
                                                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500"
                                                        />
                                                    </div>
                                                    <div>
                                                        <label className="block text-sm font-medium text-gray-700 mb-1">
                                                            Nội dung đính chính <span className="text-red-500">*</span>
                                                        </label>
                                                        <div className="rounded-lg border border-gray-300 overflow-hidden bg-white">
                                                            <ReactQuill
                                                                theme="snow"
                                                                value={addendumContent}
                                                                onChange={setAddendumContent}
                                                                placeholder="Nhập nội dung chi tiết cần đính chính..."
                                                            />
                                                        </div>
                                                    </div>
                                                    <div className="flex justify-end pt-2">
                                                        <Button 
                                                            onClick={() => {
                                                                if (!addendumReason.trim() || !addendumContent.trim() || addendumContent === '<p><br></p>') {
                                                                    toast.error('Vui lòng nhập đầy đủ lý do và nội dung đính chính');
                                                                    return;
                                                                }
                                                                createAddendumMutation.mutate({ reason: addendumReason, content: addendumContent });
                                                            }}
                                                            disabled={createAddendumMutation.isPending || !addendumReason || !addendumContent}
                                                            className="flex items-center gap-2"
                                                        >
                                                            {createAddendumMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                                                            Lưu bản đính chính
                                                        </Button>
                                                    </div>
                                                </div>
