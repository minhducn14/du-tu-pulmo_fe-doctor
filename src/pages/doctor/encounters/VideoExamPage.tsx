...
export default function VideoExamPage() {
  ...
  useEffect(() => {
    if (
      ...
      encounter.appointment.appointmentType !== "VIDEO"
    ) {
      navigate(`/doctor/encounters/${appointmentId}/in-clinic`, {
        replace: true,
      });
    }
  ...
  const handleComplete = async () => {
    try {
      ...
      toast.error(
        err?.response?.data?.message || "Không thể hoàn thành khám bệnh",
      );
    } finally {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] md:h-[calc(100vh-2rem)] flex flex-col lg:flex-row">
        ...
        {waitingForRecord ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Hồ sơ bệnh án sẽ hiển thị sau khi tham gia cuộc gọi
              </p>
            </div>
          </div>
        ) : (
          <MedicalRecordWorkspace
            ...
            headerRightSlot={
              <Button
                variant="outline"
                onClick={() => navigate("/doctor/queue-manager")}
              >
                Quay lại hàng chờ
              </Button>
            }
      ...
      {appointmentId && (
        <div className="h-[200px] lg:h-auto lg:w-[400px] shrink-0 bg-white border-slate-200 order-1 lg:order-2 overflow-hidden">
          <VideoPanel
